...
%                            SCENARIO THREE	
%==========================================================================
% the scenario three using the following options:
...
%==========================================================================
% The difference betwen the scenario one and scenario four is the limit for 
...
% and using the elevation weights.  
%==========================================================================
clear;
%--------------------------------------------------------------------------
...
file_nav  ='eph.dat';
file_obs  ='rcvr.dat';
file1     = 'satposfile_scen3.txt'; fid_file1 = fopen(file1,'w');
file2     = 'rcvposfile_scen3.txt'; fid_file2 = fopen(file2,'w');
file3     = 'satresfile_scen3.txt'; fid_file3 = fopen(file3,'w');
...
% design or coefficient matrix
            if (el(isat,1)>10)
                ...
                A(icount,5) =  1/sind(el(isat,1));                       % coefficient for wet troposphere
%--------------------------------------------------------------------------
% troposphere modelling        
                corr = tropo_error_correction(el(isat,1), walt);
% -------------------------------------------------------------------------
% computed_range vector
                computed_range(icount,1) = dist(isat,1) + clock_bias - clight*dtsat(isat,1) + 1/sind(el(isat,1))* zen_trop; 
% ------------------------------------------------------------------------
...
% -------------------------------------------------------------------------        
        ...
        for k=length(iprn_obs):-1:1
            if el(k)<10
                [iprn_obs,xsat,el,az]=reject_obs(iprn_obs,xsat,el,az,iprn_obs(k));                
            end            
        end
% -------------------------------------------------------------------------        
        % misclosure vector
